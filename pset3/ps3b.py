...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        Patient_instance = Patient(viruses, maxPop)
        for k in range(300):
            average_population[k] = average_population[k] + Patient_instance.update()
    
    # average
    for i in range(300):
        average_population[i] = average_population[i]/numTrials
    
    # plot
    pylab.plot(range(300), average_population, 'r', label='Trends')
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            check = 1
            for drug in range(len(drugResist)):
                if not virus.isResistantTo(drugResist[drug]):
                    check = 0
                    break
            if check == 1:
                total = total + 1
    ...
    def update(self):
        ...
        popDensity = float(len(self.viruses))/self.maxPop
        iteration = self.viruses.copy()
                          
        for virus in iteration:
            if virus.doesClear():
                self.viruses.remove(virus)

        for virus in iteration:
            try:
                newVirus = virus.reproduce(popDensity, self.prescription)
                self.viruses.append(newVirus)
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(300):
        average_population.append(0.0)
        resistant_population.append(0.0)
    ...
    for i in range(numTrials):
        ...
        for i in range(300):
            ...
            average_population[i] += newPatient.update()
            resistant_population[i] += newPatient.getResistPop(['guttagonol'])
            
    # average
    for i in range(300):
        average_population[i] = average_population[i]/numTrials
        resistant_population[i] = resistant_population[i]/numTrials
                          
    # plot
    pylab.plot(range(0,300), average_population, label = "Total")
    pylab.plot(range(0,300), resistant_population, label = "ResistantVirus")
