...
def generate_models(x, y, degs):
    ...
    for deg in degs:
        model.append(np.polyfit(x, y, deg))
    return model
...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.title('Climate change')
    pylab.xlabel('Year')
    pylab.ylabel('Tempature')
    
    for model in models:
        if len(model) == 2:           
            estimatedY = np.polyval(model, x)
            error = r_squared(y, estimatedY)
            pylab.plot(x, estimatedY, '-r', 
                       label = 'Fit of data with degree = 1 , R2 = ' + str(round(error, 5)))
